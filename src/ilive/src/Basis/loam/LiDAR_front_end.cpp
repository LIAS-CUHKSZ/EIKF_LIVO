...
using Msg_variant = boost::variant<sensor_msgs::PointCloud2::ConstPtr,livox_ros_driver::CustomMsg::ConstPtr>;
void mid_handler_CM(const livox_ros_driver::CustomMsg::ConstPtr &msg);
void mid_handler_pc2(const sensor_msgs::PointCloud2::ConstPtr &msg);
void mid_handle(const Msg_variant &msg );
...
int main(int argc, char **argv)
{
    ...
    case MID:
        printf("MID\n");
        sub_points = n.subscribe("/livox/lidar", 1000, mid_handler_CM, ros::TransportHints().tcpNoDelay());
        break;
...
// void mid_handle(const Msg_variant &msg ){
//     if (const livox_ros_driver::CustomMsg::ConstPtr *msg_CM = boost::get<livox_ros_driver::CustomMsg::ConstPtr>(&msg)){
//         mid_handler_CM(msg_CM);
//     }
// }
...
POINT_CLOUD_REGISTER_POINT_STRUCT(livox_mid_ros::Point,
    (float, x, x)
    (float, y, y)
    (float, z, z)
...
void mid_handler_pc2(const sensor_msgs::PointCloud2::ConstPtr &msg){
    ...
    vector<pcl::PointCloud<PointType>> pl_buff(N_SCANS);
    pcl::PointCloud<PointType> pl_corn, pl_surf,pl_full;
    pcl::PointCloud<livox_mid_ros::Point> pcl_pc;
  ...
  if (feature_enabled)
  {
    for(uint i=1; i<plsize; i++)
    {
      if((pcl_pc[i].line < N_SCANS) && ((pcl_pc[i].tag & 0x30) == 0x10 || (pcl_pc[i].tag & 0x30) == 0x00))
      {
        pl_full[i].x = pcl_pc[i].x;
        pl_full[i].y = pcl_pc[i].y;
        pl_full[i].z = pcl_pc[i].z;
        pl_full[i].intensity = pcl_pc[i].intensity;
        ...
        if((abs(pl_full[i].x - pl_full[i-1].x) > 1e-7) 
            ...
            || (abs(pl_full[i].z - pl_full[i-1].z) > 1e-7))
        {
          pl_buff[pcl_pc[i].line].push_back(pl_full[i]);
        }
      }
    }
    static int count = 0;
...
void velo16_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.curvature = pl_orig.points[i].time * 1.f; // units: ms
