...
cnt=1
for i in 'eee_02'
#  'nya_03'   'sbs_01'   'sbs_02'   'sbs_03' 'tnp_03' 'eee_01' 'eee_02'  'eee_03'  'nya_01'   'nya_02'  
do 
    ...
    do 
        ...
        do
            for m in 1
            do 
                for x in 750 1000 1250 1500 1750 2000
                do
                    echo ============================================== 
                    ...
                    roslaunch ilive NTU_VIRAL.launch  method:=$f    dataset:=$i    iters:=$j    path_save:=$output_dir   if_vio_f2f:=$m   if_vio_f2map:=$m  cam_cov_scale:=$x
                    
                    echo roslaunch finish, start saving to csv

                    eva_dir="/home/slam/workspace/ws_ILIVE/Evaluation_Data/NTU_eva/viral_eval-master/LIV${i}/${f}Iter${j}f2f${m}f2m${m}_nopho_ba10-5bg10-7n_ccs${x}"
                    if [ ! -d $eva_dir  ];then
                    mkdir -p $eva_dir
